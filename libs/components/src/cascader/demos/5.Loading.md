...
This shows load on open, load on focus, loading when scrolling to the bottom, and load on search.

# zh-CN

这里展示了打开时加载、聚焦时加载、滚动到底部时加载以及搜索时加载。
...
export default function Demo() {
  ...
  const [list3, setList3] = useImmer(
    Array.from({ length: 20 }).map((_, index1) => ({
      value: index1,
      label: `Item ${index1}`,
  ...
  const [list4, setList4] = useImmer([]);
  const [selected4, setSelected4] = useImmer(null);
          ...
          onScrollBottom={(ancestors) => {
            ...
            async.setTimeout(() => {
              ...
              setList3((draft) => {
                draft.push(
                  ...Array.from({ length: 20 }).map((_, index1) => {
                    const index = index1 + draft.length;
                    return {
                      label: `Item ${index}`,
                      value: `${index}`,
                      children: Array.from({ length: 20 }).map((_, index2) => ({
