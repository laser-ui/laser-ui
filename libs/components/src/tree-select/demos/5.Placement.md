...
```tsx
import { Compose, Radio, Separator, TreeSelect } from '@laser-ui/components';
import { useState } from 'react';

const list = Array.from({ length: 3 }).map((_, index1) => ({
  label: `Item ${index1} long long long long long long`,
  value: `${index1}`,
    ...
    <>
      ...
      <div className="row g-2">
        <div className="col-auto">
          <TreeSelect list={list} placeholder="Placement" placement={placement} />
        </div>
      </div>
    </>
  );
